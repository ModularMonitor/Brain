...
inline int MyLOG::printf_fordatalog(const e_LOG_TAG& tag, const char* format, ...)
{
    std::lock_guard<std::recursive_mutex> l(m_mtx);
    if (!Serial) Serial.begin(logger_serial_speed);
    ...
    try {
        ...
        int written = snprintf(buf_write, sizeof(buf_write) - 1, "&&&|%i|%016llu|%s|", (int)xPortGetCoreID(), get_time_ms(), tag_s);
        if (written < 0) written = 0;
